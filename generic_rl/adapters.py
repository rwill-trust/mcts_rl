...
class GameAdapter:
    ...
    def reflect_state_policy(self, state: Any, policy: 'np.ndarray') -> tuple[Any, 'np.ndarray']:
        """Return a reflected version of ``state`` and ``policy`` for augmentation.

        The default implementation simply returns a deep copy of ``state`` and a
        copy of ``policy`` unchanged. Games can override this to implement
        symmetry-based data augmentation.
        """
        if np is None:
            raise RuntimeError("NumPy is required for data augmentation")
        return self.copyState(state), policy.copy()
...
class ConnectFourAdapter(GameAdapter):
    ...
    def reflect_state_policy(self, state: Dict, policy: 'np.ndarray') -> tuple[Dict, 'np.ndarray']:
        if np is None:
            raise RuntimeError("NumPy is required for data augmentation")
        reflected_board = [row[::-1] for row in state["board"]]
        reflected_state = {
            "board": reflected_board,
            "current_player": state["current_player"],
        }
        reflected_policy = policy[::-1].copy()
        return reflected_state, reflected_policy
